        ...
        this.listeners.add(new LambdaListener<>(Render3DEvent.class, event -> {
            if (!blocks.isEmpty()) {
                for (int i = 0; i < blocks.size(); i++) {
                    try {
                        EntityPlayer enemyPlayer = findEnemy(blocks.get(i).entityID);
                        if (System.currentTimeMillis() - blocks.get(i).time > 4000
                                || Math.sqrt(DistanceUtil.distanceSq(blocks.get(i).blockPos.getX(), blocks.get(i).blockPos.getY(), blocks.get(i).blockPos.getZ(), enemyPlayer.posX, enemyPlayer.posY, enemyPlayer.posZ)) > 6.5)
