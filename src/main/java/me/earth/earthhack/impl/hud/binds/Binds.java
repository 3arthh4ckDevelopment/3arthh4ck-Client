package me.earth.earthhack.impl.hud.binds;
...
public class Binds extends DynamicHudElement {
    ...
    private void render() { //TODO: simplify
                ...
                if (onlyCombat.getValue()) {
                    if (module.getCategory() == Category.Combat) {
                        if (onlyEnabled.getValue()) {
                            if (!Objects.equals(module.getBind(), Bind.none()) && module.isEnabled() && module.getBind().getKey() > 0 ) {
                                modules.add(module);
                            }
                        } else {
    ...
    public float getHeight() {
        return Managers.TEXT.getStringHeight() * counter;
    }
