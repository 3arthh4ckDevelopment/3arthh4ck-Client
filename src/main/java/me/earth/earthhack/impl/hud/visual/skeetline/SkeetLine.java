...
public class SkeetLine extends HudElement {
    ...
    protected void onRender() {
        int endScreen = Render2DUtil.getScreenWidth() / Render2DUtil.getScreenScale();
        int midScreen = endScreen / 2;

        if (colorsNumber.getValue() == 0) {
            Render2DUtil.drawGradientRect(0, 0, midScreen, height.getValue(), true, color1.getValue().getRGB(), color1.getValue().getRGB());
            return;
        }
        ...
        for (int i = 0; i < 8; i++) {
            Render2DUtil.drawGradientRect(x + offset, 0, x + sectionSize + offset, height.getValue(), true, c1.getRGB(), c2.getRGB());
            x += sectionSize;
