    ...
    public void drawScreen(int mouseX, int mouseY, float partialTicks)
    {
        ...
        if (CLICK_GUI.get().search.getValue() == ClickGui.SearchStyle.Box) {
            ...
            if (CLICK_GUI.get().getBoxes())
                Render2DUtil.drawBorderedRect(getPosX(), getPosY(), getPosX() + getWidth(), getPosY() + getHeight(), 0.5f, CLICK_GUI.get().getModulesColor().getRGB(), CLICK_GUI.get().getTopColor().getRGB());

            Managers.TEXT.drawStringWithShadow(getLabel(), getPosX() + 3, getPosY() + getHeight() / 2 - (Managers.TEXT.getStringHeightI() >> 1), 0xFFFFFFFF);
            Render2DUtil.drawRect(getPosX(), getPosY() + getHeight(), getPosX() + getWidth(), getPosY() + getHeight() + (Managers.TEXT.getStringHeightI() + 3), 0x92000000);
            Managers.TEXT.drawStringWithShadow(text, getPosX() + 2, getPosY() - getHeight() + 33.5f, getState() ? 0xFFFFFFFF : 0xFFAAAAAA);
        } else if (CLICK_GUI.get().search.getValue() == ClickGui.SearchStyle.TextBar) {
            Render2DUtil.roundedRect(getPosX(), getPosY(), getPosX() + getWidth(), getPosY() + getHeight(), 4.0f, 0xff232323);
            Managers.TEXT.drawStringWithShadow(text, getPosX() + 10, getPosY() + getHeight() / 2 - Managers.TEXT.getStringHeightI() / 2.0f, getState() ? 0xFFFFFFFF : 0xFFAAAAAA);
        }
    }
